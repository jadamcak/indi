...
Astelco::Astelco()
{
    ...
    /**/
    SetDomeCapability(DOME_CAN_ABS_MOVE | DOME_CAN_REL_MOVE | DOME_CAN_PARK);// | DOME_CAN_SYNC
}
...
bool Astelco::initProperties()
{
    ...
    setDriverInterface(AUX_INTERFACE|DOME_INTERFACE);//|DUSTCAP_INTERFACE);
    ...
    IUFillTextVector(&StatusTP, StatusT, STATUS_COUNT, getDeviceName(), "STATUS DOME", "Status Dome", "Dome", IP_RO, 60, IPS_IDLE);
    
    IUFillNumber(&TargetPositionN[0], "GO TO", "Go To", "%4.2f", -360., 360., 0., 0.);
    IUFillNumberVector(&TargetPositionNP, TargetPositionN, 1, getDeviceName(), "GOTO", "GoTo", "Dome", IP_RW, 0, IPS_IDLE);
    IUFillSwitch(&Goto1S[0], "-10 deg", "-10 deg", ISS_OFF);
    IUFillSwitch(&Goto1S[1], "+10 deg", "+10 deg", ISS_OFF);
    IUFillSwitchVector(&Goto1SP, Goto1S, 2, getDeviceName(), "Target 10 deg", "Target", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);
    
    IUFillSwitch(&Goto2S[0], "-5 deg", "-5 deg", ISS_OFF);
    IUFillSwitch(&Goto2S[1], "+5 deg", "+5 deg", ISS_OFF);
    IUFillSwitchVector(&Goto2SP, Goto2S, 2, getDeviceName(), "Target 5 deg", "Target", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);

    IUFillSwitch(&Goto3S[0], "-1 deg", "-1 deg", ISS_OFF);    
    IUFillSwitch(&Goto3S[1], "+1 deg", "+1 deg", ISS_OFF);
    IUFillSwitchVector(&Goto3SP, Goto3S, 2, getDeviceName(), "Target 1 deg", "Target", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);
    
    IUFillNumber(&TargetOffsetN[0], "OFFSET", "Offset", "%4.2f", -360., 360., 0., 0.);
    IUFillNumberVector(&TargetOffsetNP, TargetOffsetN, 1, getDeviceName(), "TARGET OFFSET", "TargetOffset", "Dome", IP_RW, 0, IPS_IDLE);
    IUFillSwitch(&Offset1S[0], "-10 deg", "-10 deg", ISS_OFF);
    IUFillSwitch(&Offset1S[1], "+10 deg", "+10 deg", ISS_OFF);
    IUFillSwitchVector(&Offset1SP, Offset1S, 2, getDeviceName(), "Offset 10 deg", "Offset", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);
    
    IUFillSwitch(&Offset2S[0], "-5 deg", "-5 deg", ISS_OFF);
    IUFillSwitch(&Offset2S[1], "+5 deg", "+5 deg", ISS_OFF);
    IUFillSwitchVector(&Offset2SP, Offset2S, 2, getDeviceName(), "Offset 5 deg", "Offset", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);

    IUFillSwitch(&Offset3S[0], "-1 deg", "-1 deg", ISS_OFF);    
    IUFillSwitch(&Offset3S[1], "+1 deg", "+1 deg", ISS_OFF);
    IUFillSwitchVector(&Offset3SP, Offset3S, 2, getDeviceName(), "Offset 1 deg", "Offset", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);
    ...
    IUFillTextVector(&PositionTP, PositionT, POSITION_COUNT, getDeviceName(), "POSITON", "Position Limits", "Dome", IP_RO, 60, IPS_IDLE);

    IUFillSwitch(&SyncS[ON], "Sync ON", "Sync ON", ISS_OFF);    
    IUFillSwitch(&SyncS[OFF], "Sync OFF", "Sync OFF", ISS_OFF);
    IUFillSwitch(&SyncS[GET], "Get Sync", "Get Sync", ISS_OFF);
    IUFillSwitchVector(&SyncSP, SyncS, ON_OFF_COUNT, getDeviceName(), "SYNC DOME", "Sync Dome", "Dome", IP_RW, ISR_ATMOST1, 0, IPS_IDLE);

    IUFillText(&SyncT[0], "Sync Status", "Sync Status", "NA");
    IUFillTextVector(&SyncTP, SyncT, 1, getDeviceName(), "SYNC", "Sync Status", "Dome", IP_RO, 60, IPS_IDLE);
...
bool Astelco::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (!strcmp(name, TargetPositionNP.name))
        {
            ...
            return SetPosition(TargetPositionN[0].value, DOME);
        }
        ...
        if (!strcmp(name, TargetOffsetNP.name))
        {
            ...
            return SetOffsetPosition(TargetOffsetN[0].value, DOME);
        }
    }
...
bool Astelco::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    ...
    if (!strcmp(name, SyncSP.name))
    {
        IUResetSwitch(&SyncSP);
        for (int i = 0; i < SyncSP.nsp; i++)
        {
            if (states[i] != ISS_ON)
                continue;                
            if (!strcmp(SyncS[ON].name, names[i]))
            {
                if (!SetSync(true))
                {
                    SyncSP.s = IPS_ALERT;
                    LOGF_ERROR("Failed to switch to %s.","Sync Dome");
                }
                else
                {
                    SyncS[ON].s = ISS_ON;
            ...
            else if (!strcmp(SyncS[OFF].name, names[i]))
            {
                if (!SetSync(false))
                {
                    SyncSP.s = IPS_ALERT;
            ...
            else if (!strcmp(SyncS[GET].name, names[i]))
            {
                if (!GetSync())
                {
                    SyncSP.s = IPS_ALERT;
        ...
        {   if (!GetPosition(DOME))
            {
                ...
                LOGF_ERROR("Failed to get position(limits) of %s.",GetDevice(DOME));
...
void Astelco::TimerHit()
{
    ...
    if(turnedOn>=1)
    {
        GetStatus(POWER_STATE, DOME);
        GetStatus(REAL_POSITION, DOME);
        GetStatus(LIMIT_STATE, DOME);
        GetStatus(MOTION_STATE, DOME);
        GetStatus(TARGET_POSITION, DOME);
        GetStatus(OFFSET, DOME);
        GetStatus(TARGET_DISTANCE, DOME);
    }
...
void Astelco::setAnswer(const int i, const char *value)
{
    ...
    if(i > -1)
    {
        ...
        if(myenum<255)
        {
            ...
            {
                if(strcmp(value,"0")==0)
                    sprintf(history[j], "SYNC OFF");
                else if(strcmp(value,"1")==0)
                    sprintf(history[j], "SYNC ON");
                else if(strcmp(value,"3")==0)
                    sprintf(history[j], "SYNC ON DISCRETE");
                else if(strcmp(value,"5")==0)
                    sprintf(history[j], "SYNC ON CONTINUOUS");
                else if(strcmp(value,"11")==0)
                    sprintf(history[j], "SYNC ON DISC OVERRIDE");
                else
                    sprintf(history[j], "%s", value);
            } 
            else 
...
IPState Astelco::MoveAbs(double az)
{
    return MoveAbsDome_2(az);
}

IPState Astelco::MoveRel(double azDiff)
{
    return MoveRelDome_2(azDiff);
}

IPState Astelco::MoveAbsDome_2(const float deg)
{
    if(SetPosition(deg, DOME))
        return IPS_BUSY;
    return IPS_ALERT;
}

IPState Astelco::MoveRelDome_2(const float deg)
{
    if(GetStatus(REAL_POSITION, DOME))
    {
        int idx = cmdDeviceInt -1;
        sleep(5*ASTELCO_TIMEOUT);
        float new_deg = axis[history3[idx]][POSITION_COUNT] + deg;
        if(SetPosition(new_deg, DOME))
            return IPS_BUSY;
    }
    return IPS_ALERT;
}

IPState Astelco::OffsetAbsDome_2(const float deg)
{
    if(SetOffsetPosition(deg, DOME))
        return IPS_BUSY;
    return IPS_ALERT;
}

IPState Astelco::OffsetRelDome_2(const float deg)
{
     if(GetStatus(OFFSET, DOME))
    {
        int idx = cmdDeviceInt -1;
        sleep(5*ASTELCO_TIMEOUT);
        float new_deg = axis[history3[idx]][POSITION_COUNT] + deg;
        if(SetOffsetPosition(new_deg, DOME))
            return IPS_BUSY;
    }
    return IPS_ALERT;
}

bool Astelco::MoveRelDome(const float deg)
{
    return MoveRelDome_2(deg)==IPS_BUSY;
}

bool Astelco::OffsetRelDome(const float deg)
{
    return OffsetRelDome_2(deg)==IPS_BUSY;
}

IPState Astelco::Park()
{
    return MoveAbs(parkDeg);
}

IPState Astelco::UnPark()
{
    return IPS_OK;
}

bool Astelco::SetCurrentPark()
{
    int idx = cmdDeviceInt -1;
    if(GetStatus(REAL_POSITION, DOME))
    {        
        sleep(5*ASTELCO_TIMEOUT);
        parkDeg = axis[history3[idx]][POSITION_COUNT];
    }
    return (parkDeg == axis[history3[idx]][POSITION_COUNT]);
}

bool Astelco::SetDefaultPark()
{
    parkDeg = parkDefDeg;
    return (parkDeg == parkDefDeg);
}

bool Astelco::SetSync(bool set)
{
    char cmd[254] = {0};
    int value {0};
    if(set)
        value = 1+4; //continuous tracking
    sprintf(cmd, "%d %s%d", cmdDeviceInt, "SET POINTING.SETUP.DOME.SYNCMODE=", value);
    setHistories(SyncT[0].text, 40);
    bool succes = sendCommand(cmd);//increments cmdDeviceInt internally
    return succes;
}

bool Astelco::GetSync()
{
    char cmd[254] = {0};
    sprintf(cmd, "%d %s", cmdDeviceInt, "GET POINTING.SETUP.DOME.SYNCMODE");
    setHistories(SyncT[0].text, 40);
    bool succes = sendCommand(cmd);//increments cmdDeviceInt internally
    return succes;
}