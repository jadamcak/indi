...
void* Astelco::func(void *)
{
    LOGF_INFO("Child thread Created: %s", "123");
    char resp[256] = {0};
    char value[256] = {0};
    int cmd = 0;
    while(threadOn)
    ...
    LOGF_INFO("Child thread Destroyed: %s", "123");    
}

bool Astelco::createThread()
{
    if(pthread_create(&Astelco_t1, NULL, &func, NULL))
        return false;
    if(pthread_detach(Astelco_t1))
        return false;
    threadOn = true;
    return threadOn;
}
